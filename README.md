...
| `redirectUris` | SS | Registered redirect URIs. Not enforced yet: a request with an unregistered URI logs `[redirect] client … redirect_uri … not registered` so values can be collected first. Once enforced, the match is exact (RFC 6749 simple string comparison); carry per-login data in `state`, not in the URI. |
